...
def _import_streamvggt():
    """Lazily import GHOST's causal StreamVGGT geometry encoder.

    GHOST is a sibling repo that uses absolute ``streamvggt.*`` imports, so its
    ``src/`` directory must be on ``sys.path``. Resolution order:
    ``$GHOST_SRC`` env var, then the standard repo-sibling layout. Imported
    lazily so the default (VGGT) path has no dependency on GHOST being present.
    """
    import os
    import sys

    candidates = []
    env = os.environ.get("GHOST_SRC")
    if env:
        candidates.append(env)
    here = os.path.dirname(os.path.abspath(__file__))
    candidates.append(os.path.join(here, "../../../../GHOST/src"))  # CoRL-ICRA/GHOST/src
    for cand in candidates:
        cand = os.path.abspath(cand)
        if os.path.isdir(os.path.join(cand, "streamvggt")):
...
class Qwen2_5_VLForConditionalGenerationForJanusVLN(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # Lever 1 (geometry): "vggt" = original offline-trained VGGT (baseline);
        # "ghost"/"streamvggt" = GHOST's causal StreamVGGT encoder. Both are frozen
        # and expose the identical aggregator interface used by the fusion path.
        self.geometry_encoder_type = getattr(config, "geometry_encoder", "vggt")
        if self.geometry_encoder_type in ("ghost", "streamvggt"):
            StreamVGGT = _import_streamvggt()
            vggt = StreamVGGT(img_size=518, patch_size=14, embed_dim=1024)
        else:
            vggt = VGGT()
        vggt.camera_head = None
        vggt.track_head = None
        # StreamVGGT also carries depth/point heads; drop them — the geometry tap is
        # the aggregator tokens (output_list[-2]), identical in layout to VGGT.
        for _head in ("point_head", "depth_head"):
            if getattr(vggt, _head, None) is not None:
                setattr(vggt, _head, None)
        self.vggt = vggt
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, mode=None, **kwargs):
        ...
        streamvggt_model_path = kwargs.pop("streamvggt_model_path", None)
        model = super().from_pretrained(pretrained_model_name_or_path, *model_args, **kwargs)
        model.mode = mode
        if getattr(model, "geometry_encoder_type", "vggt") in ("ghost", "streamvggt"):
            # GHOST ships a flat state_dict checkpoint (lch01/StreamVGGT: ckpt/model.pth),
            # not an HF directory, so load it directly into the (already-built) encoder.
            if streamvggt_model_path:
                print(f"Loading StreamVGGT weights from {streamvggt_model_path}")
                sd = torch.load(streamvggt_model_path, map_location="cpu")
                sd = sd.get("model", sd.get("state_dict", sd)) if isinstance(sd, dict) else sd
                missing, unexpected = model.vggt.load_state_dict(sd, strict=False)
                print(f"StreamVGGT load: {len(missing)} missing, {len(unexpected)} unexpected keys")
            for param in model.vggt.parameters():
                param.requires_grad = False
        elif vggt_model_path:
